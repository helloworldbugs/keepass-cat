...
function LocalChromePasswordFileManager(settings) {
  ...
  function listDatabases() {
    return Promise.all(savingLocks)
      .then(function () {
  ...
  function getChosenDatabaseFile(dbInfo) {
    return listDatabases().then(function (databases) {
      ...
      else throw new Error('Failed to find the requested file');
    });
  }
  ...
  function saveDatabase(db) {
    ...
    var p = listDatabases().then(function (existingFiles) {
      var index = existingFiles.reduce(function (prev, curr, index) {
        if (curr.title == db.title) return index;
        else return prev;
      }, -1);
  ...
  function deleteDatabase(db) {
    return listDatabases().then(function (databases) {
      ...
      else return chromePromise.storage.local.remove('passwordFiles');
    });
  }
